...
if (array_key_exists('upload', $_POST)) {  //Αν έχει πατηθεί το κουμπί "Εισαγωγή".
	...
	//              Επειδή μιλάμε για Update το απενεργοποιώ
	//---------------------------------------------------------------------------
	/*
	include ('db_main.php');
	...
	//---------------------------------------------------------------------------
	//         Διαγραφή της φωτογραφίας της παλιάς φωτογραφίας αρχικά.
	//---------------------------------------------------------------------------
	...
	$old_photo=$_GET['isbn'].".jpg";
	fileDelete('../download/',$old_photo);
	//---------------------------------------------------------------------------
	...
	*/
	...
	if ($_POST['z1']==""){
		...
		$alert = "Πρέπει να εισάγεις 3 αριθμούς ακριβώς στο πεδίο Κωδικός.";		
	...
	*/
	...
	//----------------------------------------------------------------------	
		//$_FILES['image']['name']=$_POST['z1'].".jpg";  Αυτό στην επεξεργασία βγάζει λάθος γιατί δεν υπάρχει postz1 γιατί είναι σταθερό το isbn δεν αλλάζει 
		//                                               άρα δεν το στέλνουμε.
		
		$_FILES['image']['name']=$_GET['isbn'].".jpg";   // Γιαυτό για να ονοματίσω την καινούργια φωτογραφία τσιμπάω το isbn.
...
//------------------------------------------------------------------------------------------------------------------------------------------
		...
		//} else { 
			...
			*/
			...
			$sql = 'UPDATE books SET author =       "'.$_POST['z2'].'",
									 title =        "'.$_POST['z3'].'",
									 catid =        "'.$_POST['abc'].'",
									 subcatid =     "'.$_POST['abc2'].'",
									 price =        "'.$_POST['z4'].'",
									 presentation = "'.$_POST['a'].'",
									 offer =        "'.$_POST['b'].'",
									 description =  "'.$_POST['z5'].'",
									 picture =      "'. "../download/"  .  $_FILES['image']['name'] .'" ,
									 orientation=   "'.$_POST['orientation'].'"								 
					';        													
					
			$sql .= 'WHERE isbn = '."'$isbn2'";	
			...
			if ($result) {
				$alert_final = "<b>Εγινε η εισαγωγή.</b>";
...
<div id="frame">
	...
	<div id="columnRight">
			...
			<div class="periheader">
				<h1>Επεξεργασία ενός Προϊόντος</h1>   <!-- ------------------------------------------------------------------------------------------------ -->
			</div>	
	<?php
		//if ($_GET && !$_POST) {
			//print_r($_GET);
	?>		
			...
			<div id="red_alert">  <?php if(isset($alert_final)) echo "Προσοχή : ".$alert_final; ?>   </div>
			...
			$isbn2=$_GET['isbn'];
			include ('db_main.php');
			$sql_AllPage = "SELECT * FROM books  WHERE isbn = "."'$isbn2'";				  
			$result_AllPage = $db->query($sql_AllPage);
			$numrows_AllPage = $result_AllPage->num_rows;						
				
			while ($row_AllPage = $result_AllPage->fetch_assoc() ) {
				
			//echo "-----------".$row_AllPage['isbn'];			
			?>
			...
			<td bgcolor="F0E1FA"><?php  // ------------------------  Πρώτο ComboBox : Κατηγορία ---------------------------------------
				...
				include ('db_main.php');
				$sql41='SELECT * FROM categories ';
				$result41 = $db_41->query($sql41);
				
				$numrows41 = $result41->num_rows;
				?>
				...
				while ($row41 = $result41->fetch_assoc()) {					
					...
					if (array_key_exists('show', $_POST)) {
						...
						echo "<option value=".$row41['catid']." > ".$row41['catname']." </option>";
					}					
				}
			...
			<td bgcolor="F0E1FA"><input type="submit" name="z1" size="3" maxLength="3" onkeypress='return isNumberKey(event)' 
					   value="<?php if (isset($_POST['z1'])){ echo $_POST['z1']; } else { echo $row_AllPage['isbn']; } ?>" class="product_insert_small"></td>
			<?php
			if (isset($result11) ) {				
			echo "<td bgcolor='82E9AF' border='0' width='700'>";
			...
			<td bgcolor="EDFA73"><input type="text" name="z2" size="40" maxLength="40"
			value="<?php if (isset($_POST['z2'])) { echo $_POST['z2'];  } else {  echo $row_AllPage['author']; } ?>" class="product_insert_big">
			</td>
			<?php
			if (isset($result11) ) {				
			...
			value="<?php if (isset($_POST['z3'])){ echo $_POST['z3'];} else { echo $row_AllPage['title']; } ?>" class="product_insert_big"></td>
			<?php
			if (isset($result11) ) {				
			echo "<td bgcolor='E9CE82' border='0' width='700'>";
			...
			value="<?php if (isset($_POST['z4'])){ echo $_POST['z4']; } else { echo $row_AllPage['price']; }?>" class="product_insert_small"></td>
			<?php
			if (isset($result11) ) {				
			echo "<td bgcolor='E9CE82' border='0' width='700'>";
			...
			<td bgcolor="F0E1FA"><textarea name="z5" rows="10" cols="40" class="product_insert">
			<?php if (isset($_POST['z5'])) { echo $_POST['z5']; } else { echo $row_AllPage['description']; } ?>
			</textarea>
			</td>
			<?php
			if (isset($result11) ) {				
			...
			*/	
			...
			<input name="upload"        type="submit"   value="Εισαγωγή">			
			<input name="insPublisher1" type="reset"    value="Καθαρισμός Φόρμας"></td>
			</tr>
